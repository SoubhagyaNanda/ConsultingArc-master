...
<head>
<meta charset="UTF-8">
<title>Edit Consultant | Consultant</title>
...
<body>
	...
	<!-- check is employee is login or not -->
		...
		<div class="row">
						...
						<form action="../employeeChangePassword" method="post">
							<div class="mb-3">
								<label class="form-label">Enter New Password</label> <input
									...
									required
									title="Minimum eight characters, at least one upper placeholder="
									Enter password" class="form-control" required="required">

							</div>
							<div class="mb-3">
								<label class="form-label">Enter Old Password</label> <input
									name="oldPassword" type="password"
									placeholder="Enter old password"  pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$"								  
								 required title="Minimum eight characters, at least one upper placeholder="Enter password" class="form-control" required>
							</div>
							<input type="hidden" value="${employeeObj.id}" name="doctorId">

							<button type="submit" class="btn btn-success col-md-12"
								style="background-color:teal; border-radius: 80px;">Change
								Password</button>
						</form>
						<!-- employee change password form -->
					</div>
				</div>
			...
			<div class="col-md-6  offset-md-2">
				<div class="card my-card">
					...
					<div class="card-body">
						<p class="fs-3 text-center text-success">Edit Consultant
							Profile</p>
						...
						<!-- boostrap form -->
						<form action="../employeeEditProfile" method="post">
							<div class="mb-3">
								<label class="form-label">Full Name</label> <input
									name="fullName" type="text" placeholder="Enter full name"
									class="form-control" value="${employeeObj.fullName}">

							</div>
							<div class="mb-3">
								<label class="form-label">Date of Birth</label> <input
									name="dateOfBirth" type="date" placeholder="Enter DOB"
									class="form-control" value="${employeeObj.dateOfBirth}">
							...
							<div class="mb-3">
								<label class="form-label">Qualification</label> <input
									...
									value="${employeeObj.qualification}">
							</div>

							<div class="mb-3">
								<label class="form-label">Specialist</label> <select
									class="form-control" name="specialist">
									<option>${ employeeObj.specialist }</option>
							...
							<div class="mb-3">
								<label class="form-label">Email address</label> <input
									...
									class="form-control" readonly value="${employeeObj.email}">

							</div>
							<div class="mb-3">
								<label class="form-label">Phone</label> <input name="phone"
									type="text" placeholder="Enter mobile number"
									class="form-control" value="${employeeObj.phone}">
							...
							<button type="submit"
								class="btn btn-success text-white col-md-12"
								style="background-color:teal; border-radius: 80px;">Update</button>
						</form>
